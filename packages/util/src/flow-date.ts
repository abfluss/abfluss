...
export class FlowDate {
    ...
    public distance(diffDate: FlowDate): number {
        const timespanOneDay: number = 1000 * 60 * 60 * 24;
        const dateInMillis1: number = this.date.getTime();
        const dateInMillis2: number = diffDate.date.getTime();
        const diffInMillis: number = dateInMillis2 - dateInMillis1;
